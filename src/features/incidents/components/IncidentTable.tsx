...
import { Table, Box, Text, Link, Checkbox, Flex, Icon } from '@chakra-ui/react';
...
export const IncidentTable = ({
    ...
    const handleSelectAll = (details: { checked: boolean | string }) => {
        const isChecked =
            typeof details.checked === 'string' ? details.checked === 'true' : details.checked;
            ...
            <Table.Root variant="outline">
                <Table.Header bg="gray.50">
                    <Table.Row>
                        ...
                        >
                            <Flex align="center">ID {renderSortIcon('id')}</Flex>
                        </Table.ColumnHeader>
                        <Table.ColumnHeader
                            cursor={onSort ? 'pointer' : 'default'}
                            onClick={() => handleHeaderClick('title')}
                        >
                            <Flex align="center">Заголовок {renderSortIcon('title')}</Flex>
                        </Table.ColumnHeader>
                        ...
                        >
                            <Flex align="center">Репортер {renderSortIcon('reporter')}</Flex>
                        </Table.ColumnHeader>
                        <Table.ColumnHeader
                            cursor={onSort ? 'pointer' : 'default'}
                            onClick={() => handleHeaderClick('createdAt')}
                        >
                            <Flex align="center">Дата создания {renderSortIcon('createdAt')}</Flex>
                        </Table.ColumnHeader>
                    </Table.Row>
                </Table.Header>
                <Table.Body>
                                    ...
                                    <Table.Cell>
                                        <Checkbox.Root
                                            ...
                                            onCheckedChange={(details) =>
                                                handleSelectOne(incident.id, details)
                                            }
                                        >
                                            <Checkbox.HiddenInput />
                                            <Checkbox.Control />
